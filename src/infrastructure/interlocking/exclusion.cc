...
struct exclusion_set {
  ...
  bool subset(exclusion_set const& other) const {
    std::ignore = other;
    return false;
  }

  bool superset(exclusion_set const& other) const {
    std::ignore = other;
    return false;
  }
  ...
  cista::offset::bitvec bits_{};
};

soro::vector<exclusion_set> get_interlocking_exclusion_sets(
    infrastructure_t const& infra) {
  static const type_set POI_TYPES = {type::SIMPLE_SWITCH, type::CROSS};

  soro::vector<soro::vector<interlocking_route::id>> expanded_sets(
      infra.graph_.elements_.size());

  for (auto const& ir : infra.interlocking_.routes_) {
    for (auto const& rn : ir.iterate(infrastructure(&infra))) {
      if (POI_TYPES.contains(rn.node_->type())) {
        expanded_sets[rn.node_->element_->id()].emplace_back(ir.id_);
      }
    }
  }

  for (auto& set : expanded_sets) {
    utls::sort(set);
  }

  auto sets =
      soro::to_vec(expanded_sets, [](auto&& s) { return exclusion_set(s); });
...
get_interlocking_route_exclusion_sets(infrastructure_t const& infra) {
  soro::vector<soro::vector<exclusion_set_id>> exclusion_sets;
