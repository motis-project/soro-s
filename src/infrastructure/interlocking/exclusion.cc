...
soro::vector<section::id> get_sections(interlocking_route const& ssr,
  ...
  for (auto const [e1, e2] : utl::pairwise(section_elements)) {
    ...
    // two elements are connected more than once, so there exist more than one
    // connecting section. Appraoch: Just check which section contains a track
    ...
    // this means it only contains track elements facing the opposite direction
    it = utls::find_if(connecting_sections, [&, e = e1](auto&& section_id) {
      auto const& section = iss.graph_.sections_[section_id];
    ...
    utl::verify(it != std::cend(connecting_sections),
                "Could not find a suitable connection in connecting sections");
...
soro::vector<element_ptr> get_exclusion_elements(
  ...
  auto const handle_single_section_ssr = [&]() {
    ...
    auto start = utls::find(section.elements_, ssr.nodes().front()->element_);
    auto end = utls::find(section.elements_, ssr.nodes().back()->element_);
...
soro::vector<soro::vector<ir_ptr>> get_ssr_conflicts(
    base_infrastructure const& iss, interlocking_subsystem const& ssr_m) {
