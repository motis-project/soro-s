...
ordering_graph::ordering_graph(infra::infrastructure const& infra,
  ...
  for (auto const& train : tt->trains_) {
    for (auto const& midnight : train.departures(interval)) {
      ...
      auto const stamps = runtime_calculation(
          train, infra, {type::APPROACH_SIGNAL, type::MAIN_SIGNAL});

      utls::sasserts([&]() {
        auto const ms_count = utls::count_if(stamps.times_, [](auto&& t) {
      ...
      for (auto const [idx, ir_id] : utl::enumerate(train.path_)) {
        ...
        soro::absolute_time const from = INVALID<absolute_time>;
        soro::absolute_time const to = INVALID<absolute_time>;

        for (auto const es_id :
             infra->exclusion_.irs_to_exclusion_sets_[ir_id]) {
          route_orderings[es_id].push_back(
  ...
  for (auto const& usage_order : route_orderings) {
    for (auto [from, to] : utl::pairwise(usage_order)) {
      nodes_[from.node_id_].out_.emplace_back(to.node_id_);
      nodes_[to.node_id_].in_.emplace_back(from.node_id_);
    }
  }
  ...
  //  // usage of an interlocking route with the smallest from_ time value of
  //  its
