...
std::vector<raw_train::run> get_raw_train_runs(utls::loaded_file const&,
  ...
  //  };
  //
  //  utl::line_range{utl::buf_reader{runs_file.contents_}} |
  //  utl::csv<run_row>() |
  //      utl::for_each(
  //          [&](auto&& row) {
  ...
  //
  //            auto station_it =
  //            infra->ds100_to_station_.find(row.ds100_.val()); if (station_it
  //            == std::end(infra->ds100_to_station_)) {
  //              runs[row.id_] = {};
  //              uLOG(utl::warn)
  ...
  //                global_minimum_stop_time, (departure -
  //                arrival).as_duration());
...
std::vector<raw_train> get_raw_trains(utls::loaded_file const&) {
