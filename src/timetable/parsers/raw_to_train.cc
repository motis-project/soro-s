...
soro::vector<soro::unique_ptr<train>> raw_trains_to_trains(
    ...
    infra::station_route_graph const&, rolling_stock const&) {
  ...
  //
  //    tr->path_ = get_ssr_run_path(rt.run_.routes_, rt.freight_, ssr_man,
  //    srg);
  //
  ...
  //
  //      tr->stop_times_[idx] = {
  //          .arrival_ = arr, .departure_ = dep, .min_stop_time_ =
  //          min_stop_time};
  //    }
  //
  //    utls::sassert(
  ...
  //                  "For now every station can only appear once in a train
  //                  run");
  //
