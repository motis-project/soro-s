...
void test_section_with_zero_length(section const& sec) {
  CHECK_EQ(sec.elements_.size(), 2);
  auto const first = sec.elements_.front();
  auto const last = sec.elements_.back();
  CHECK_EQ(first->get_km(last), last->get_km(first));
}
...
void test_station_route_lengths(infrastructure const& infra) {
  for (auto const& sr : infra->station_routes_) {
    ...
    auto const e2 = get_path_length_from_elements(utls::coro_map(
        sr->iterate(skip_omitted::OFF), [](auto&& rn) { return rn.node_; }));
    auto const e3 = get_path_length_from_elements(utls::coro_map(
        sr->iterate(skip_omitted::ON), [](auto&& rn) { return rn.node_; }));
...
void test_train_path_lengths(timetable const& tt, infrastructure const& infra) {
  for (auto const& train : tt) {
    auto const e1 = get_path_length_from_elements(
        utls::coro_map(train->iterate(skip_omitted::OFF, infra),
                       [](auto&& rn) { return rn.node_; }));

    auto const s1 = get_path_length_from_sections(
        utls::coro_map(train->iterate(skip_omitted::ON, infra),
                       [](auto&& rn) { return rn.node_; }));
...
TEST_SUITE("path length") {
  ...
  TEST_CASE("train path lengths") {  // NOLINT
    ...
    test_train_path_lengths(tt, infra);
  }
}
