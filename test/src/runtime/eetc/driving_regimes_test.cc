...
train_physics generate_test_train() {
  ...
  resistance_curve_t resistance_poly =
...
TEST_CASE("physics::accelerate_reverse::neg") {  // NOLINT
  ...
  si::acceleration acc = {-1.0F};
  si::speed vel1 = {0.0F};
  si::length start{si::ZERO<si::length>};
  si::length end = {1.0F};
  ...
  si::speed vel0 = {static_cast<float>(
...
TEST_CASE("general_driving::simulate::cruising::normal") {  // NOLINT
  ...
  std::vector<interval> intervals = {interval(si::length{15.0F}, {62.0F})};
...
TEST_CASE("general_driving::get_speed_range::empty") {  // NOLINT
  ...
  std::vector<runtime_result> is_rr = {};
