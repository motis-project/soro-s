...
TEST_CASE("t") {  // NOLINT
  ...
  auto tt = timetable(FOLLOW_OPTS, infra);

  sim_graph const sg(infra, tt);
  std::cout << "ORder Edges\n";
  for (auto const& sn : sg.nodes_) {
    for (auto const& e : sn.out_) {
      std::cout << sn.id_ << " " << e << '\n';
    }
  }

  simulation_options const sim_opts;
  auto const sim_results = simulate(sg, sim_opts);
