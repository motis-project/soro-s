...
export const SettingsStore: Module<SettingsState, unknown> = {
  ...
  mutations: {
    ...
    setTheme(this: VuetifyExtension, state, theme) {
      state.theme = theme;
      // we need to set the theme globally in vuetify
      // to access its properties in components
      this.$vuetify.theme.global.name.value = theme;
    },

    setPrimaryColor(this: VuetifyExtension, state, primaryColor) {
      state.primaryColor = primaryColor;
      // we need to set the primary color globally in vuetify
      // to access its properties in components
      const themes = this.$vuetify.theme.themes.value;
      Object.keys(themes).forEach(
        (themeKey) => (themes[themeKey].colors.primary = primaryColor)
      );
    },

    restoreVuetifyThemePrimaryColor(this: VuetifyExtension, state) {
      state.primaryColor =
        this.$vuetify.theme.global.current.value.colors.primary;
    }
  },
  ...
  actions: {
    async loadSettings({ dispatch, commit }) {
      ...
      const darkLightModePreference = storage.getItem(
    ...
    initThemeListener({ commit, state }) {
      ...
      themeMediaQuery.addEventListener('change', (event) => {
        ...
        commit(
          'setTheme',
          event.matches ? DarkLightModes.DARK : DarkLightModes.LIGHT
        );
      });
    },
    ...
    applyTheme({ commit, state }) {
      ...
      commit(
        'setTheme',
        themeMediaQuery.matches ? DarkLightModes.DARK : DarkLightModes.LIGHT
      );
    },
