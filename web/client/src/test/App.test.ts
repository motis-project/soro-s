...
describe('App', async () => {
  ...
  it('contains a theme provider that provides the theme set in the state to sidebar and golden layout', async () => {
    ...
    const navigation = themeProvider.findComponent({ name: 'soro-sidebar' });
    const goldenLayoutAdapter = themeProvider.findComponent({
      name: 'golden-layout-adapter'
    });
  ...
  it('initializes the entire app context when it is mounted', async () => {
    ...
    expect(
      goldenLayoutStoreMock.mutations.setRootComponent
    ).toHaveBeenCalledWith(expect.any(Object), goldenLayoutRoot.vm);
          ...
          content: [
            {
              ...
              content: [
                {
                  title:
                    GLComponentTitles[ComponentTechnicalName.INFRASTRUCTURE],
                  type: 'component',
                  componentType:
