import {
  ...
  iconExtension,
  addIcons
} from '@/components/infrastructure/add-icons';
...
describe('add-icons', async () => {
  ...
  beforeEach(async () => {
    map.loadImage.mockImplementation(
      (url: string, callback: (error: string, image: any) => void) =>
        callback('', {})
    );
  ...
  describe('addIcons()', async () => {
    it.each(ElementTypes)(
      'returns a promise that loads the icon for %s into the map',
      async (elementType: string) => {
        ...
        expect(map.addImage).toHaveBeenCalledWith(`icon-${elementType}`, {
          some: 'image',
          foo: 'bar'
        });
      }
    ...
    it("rejects the promise with the given error when an error is given to the callback from 'loadImage'", async () => {
      map.loadImage.mockImplementation(
        (url: string, callback: (error: string, image: any) => void) =>
          callback('some-dubious-error', { some: 'error-image' })
      );
    ...
    it("rejects the promise with default error when no error is given to the callback from 'loadImage' but no image was loaded", async () => {
      map.loadImage.mockImplementation(
        (
          url: string,
          callback: (error: string | undefined, image: any) => void
        ) => callback(undefined, undefined)
      );

      await expect(addIcons(map)).rejects.toThrow(
        'Image was not loaded but error was not thrown'
      );
    });
  });
});
