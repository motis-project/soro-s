...
describe('infrastructure-legend', async () => {
  ...
  it('displays a button to expand and collapse the legend', async () => {
    const collapseButton = infrastructureLegend.findComponent({
      name: 'v-btn'
    });
    ...
    expect(mapLegend.classes()).not.toContain(
    ...
    expect(mapLegend.classes()).toContain(
      'infrastructure-map-legend-collapsed'
    );
  ...
  it("displays a reset link that emits 'reset' upon being clicked", async () => {
    const reset = infrastructureLegend.find<HTMLDivElement>(
  ...
  it.each(Array.of(...ElementTypes, ...SpecialLegendControls))(
    'displays a checkbox for the legend control %s',
    async (elementType: string) => {
      const mapLegend = infrastructureLegend.findComponent({
        ref: 'mapLegend'
      });
  ...
  it('exports special legend controls available for other modules', async () => {
    ...
    expect(SpecialLegendControls).toStrictEqual(['Rising', 'Falling']);
  });

  it('initially checks the checkboxes given in the props', async () => {
    await infrastructureLegend.setProps({
      checkedControls: [ElementType.STATION, ElementType.HALT]
    });
    const mapLegend = infrastructureLegend.findComponent({ ref: 'mapLegend' });

    const stationCheckbox = mapLegend.find<HTMLInputElement>(
      'input[value="station"]'
    );
    ...
    const bumperCheckbox = mapLegend.find<HTMLInputElement>(
      'input[value="bumper"]'
    );
