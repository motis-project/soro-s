...
describe('menu-settings', async () => {
  ...
  it('displays a button that can open the color picker', async () => {
    const colorPickerContainer = menuSettings.findComponent(
      '.accent-color-picker'
    );
    const toggleButton = colorPickerContainer.findComponent({ name: 'v-btn' });

    expect(
      colorPickerContainer.findComponent({ name: 'v-color-picker' }).exists()
    ).toBe(false);
  ...
  it("updates the color selection when the color picker emits an 'update' event", async () => {
    ...
    await colorPickerContainer
      .findComponent({ name: 'v-btn' })
      .trigger('click');

    const colorPicker = colorPickerContainer.findComponent({
      name: 'v-color-picker'
    });
