...
vi.mock('golden-layout', async () => {
  const originalGoldenLayout = await vi.importActual<typeof GoldenLayout>(
    'golden-layout'
  );
  ...
  return {
    ...
    VirtualLayout: vi.fn((...args) => {
      const virtualLayoutInstance: any = new originalGoldenLayout.VirtualLayout(
        ...args
      );
...
const getLastVirtualLayoutInstance = () =>
  (VirtualLayout as unknown as Mock).mock.results.at(-1)?.value;
...
describe('golden-layout-adapter', async () => {
  ...
  describe('addGLComponent()', async () => {
    it('creates a golden layout component containing the specified component with its default title', async () => {
      ...
      const glComponents = adapter.findAllComponents({
        name: 'golden-layout-component'
      });
    ...
    it('uses the given title if a title is given', async () => {
      await adapter.vm.addGLComponent(
        ComponentTechnicalName.SIMULATION,
        'some-fancy-title'
      );
    ...
    it('reuses old reference ids for components from previous unbound components', async () => {
      ...
      const usedContainer =
        virtualLayoutInstance.bindComponentEvent.mock.calls[0][0];
  ...
  describe('loadGLLayout()', async () => {
    ...
    it('loads a previously saved layout config transformed from resolved form', async () => {
      await adapter.vm.addGLComponent(
        ComponentTechnicalName.ORDERING_GRAPH,
        'some-title'
      );
      ...
      expect(virtualLayoutInstance.loadLayout).toHaveBeenCalledWith(
        LayoutConfig.fromResolved(layout)
      );
    });
    ...
    it.each([
      ...
      'loads a complex layout config with refIds for the components when %s',
      async (
        _: string,
  ...
  it('allows getting the layout config via the golden layout instance', async () => {
    virtualLayoutInstance.saveLayout.mockImplementationOnce(() => ({
      some: 'layout'
    }));
  ...
  it('handles virtual recting required events on containers by setting the pos and size on their component', async () => {
    ...
    const usedContainer =
      virtualLayoutInstance.bindComponentEvent.mock.lastCall[0];
    ...
    const glComponent = (
      adapter.vm.$refs['glc_0'] as (typeof GoldenLayoutComponent)[]
    )[0];
    vi.spyOn(glComponent, 'setPosAndSize');

    virtualLayoutInstance.beforeVirtualRectingEvent();
    usedContainer.virtualRectingRequiredEvent(usedContainer, 10, 20);

    const glRootBoundingRect = (
      adapter.find({ ref: 'GLRoot' }).element as HTMLElement
    ).getBoundingClientRect();
    const containerBoundingRect = (
      usedContainer.element as HTMLElement
    ).getBoundingClientRect();
  ...
  it.each(
    Object.keys(ComponentTechnicalName).map((key) => ({
      componentTechnicalName: key as ComponentTechnicalName,
      componentName: GLComponentNames[key as ComponentTechnicalName]
    }))
  )(
    'GL component mount test %#: $componentName',
    async ({ componentTechnicalName, componentName }) => {
      ...
      expect(glComponents).toHaveLength(1);
      const subComponent = glComponents[0].findComponent({
        name: componentName
      });
