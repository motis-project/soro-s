...
int main(int argc, char const** argv) {
  ...
  for (auto const& infra_file : infra_todo) {
      ...
      //Uses OSM file located in resources/OpenStreetMap/ called germany_filtered.osm
      pugi::xml_parse_result error = osmData.load_file("../../resources/OpenStreetMap/germany_filtered.osm");

      if (error) {
          stationData.load_file(osm_file.c_str());

          /*
          * This adds the new OSM data to the OSM file generated by the infrastructur
          * In order to concatinate these files, one has to have its head removed 
          * (<osm version="0.6"> ...)  and the other one needs to have its end 
          * removed (</osm>)
          */
          ...
          combinedMapData.load(combinedMapDataStream);
          //save to file
