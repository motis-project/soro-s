  ...
  TEST_CASE("interpolation") {
    ...
    for (auto const& station : infra->stations_) {
      for (const border& border : station->borders_) {
        ...
        CHECK(interpolation.first_elem_ == border.element_->id());
        CHECK(interpolation.second_elem_ == border.neighbour_element_->id());
        for (auto bezier_elem : interpolation.points_) {
          CHECK(typeid(bezier_elem).name() == typeid(gps).name());
        }
      }
    }
  }
